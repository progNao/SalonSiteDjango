...
class MonthCalendarMixin(BaseCalendarMixin):
    ...
    def get_current_month(self):
        ...
        if month and year:
            month = datetime.datetime(year=int(year), month=int(month), day=1)
        else:
            month = datetime.datetime.today().replace(day=1)
        return month
...
class MonthWithScheduleMixin(MonthCalendarMixin):
    ...
    def get_month_schedules(self, start, end, days):
        ...
        for schedule in queryset:
            schedule_date = getattr(schedule, self.date_field).date()
